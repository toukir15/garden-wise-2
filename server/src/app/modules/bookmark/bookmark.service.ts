...
const getBookmarkFromDB = async (bookmarkId: string, page: number = 1, limit: number = 5) => {
  ...
  const allPosts = result.posts as any[]
  const total = allPosts.length
  const savedPostIds = allPosts.map((p: any) => p._id?.toString())
